...
static int set_tfa9887l_spkamp(int en, int dsp_mode);
	   ...
	   unsigned long arg)
{
	...
	switch (cmd) {
	case TFA9887_SET_SPKAMP:
                pr_debug("%s: TFA9887_SET_SPKAMP\n", __func__);
                ...
                len = reg_value[0];
                set_tfa9887l_spkamp(reg_value[1],0);
                break;
	...
	case TFA9887_KERNEL_LOCK:
		...
		if (reg_value[1]) {
			...
			if (lock_from_userspace >= 0) {
				...
				pr_err("%s: lock_from_userspace is not equal to zero, should not meet."
	...
	case TFA9887_KERNEL_INIT:
		pr_debug("%s: TFA9887_KERNEL_INIT (L) ++ kernel count %d\n",
			__func__, atomic_read(&(spk_ampl_lock.count)));
		while (lock_from_userspace > 0) {
			pr_err("%s: TFA9887_KERNEL_INIT (L) lock count from userspace %d != 0, unlock it\n",
		...
		pr_err("%s: TFA9887_KERNEL_INIT (L) -- kernel count %d\n",
			__func__, atomic_read(&(spk_ampl_lock.count)));
		break;
...
int tfa9887l_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret) {
        ...
        if(ret < 0)
                ...
                pr_err("%s:i2c read successfully\n",__func__);
