...
static int set_tfa9887_spkamp(int en, int dsp_mode);
	   ...
	   unsigned long arg)
{
	...
	case TFA9887_SET_SPKAMP:
		...
		len = reg_value[0];
		set_tfa9887_spkamp(reg_value[1],0);
		break;
	...
	case TFA9887_KERNEL_LOCK:
		...
		pr_err("%s: TFA9887_KERNEL_LOCK %d\n", __func__, reg_value[1]);
		if (reg_value[1]) {
			mutex_lock(&spk_amp_lock);
			lock_from_userspace++;
		} else {
	...
	case TFA9887_KERNEL_INIT:
		pr_err("%s: TFA9887_KERNEL_INIT ++ kernel count %d\n",
